...
```ts
new DownloadTask(options: DownloadOptions): DownloadTask
```
#### Parameters
...
```ts
map(transform: Transform<DownloadTask, N>): Task<N>
```
#### Parameters

- **transform**: `Transform<DownloadTask, N>`
#### Return Type
...
```ts
setName(name: string, param: object): DownloadTask
```
#### Parameters
